...
def open_editor_window(
    ...
    global _pending_html, _pending_title, _pending_on_close
    _pending_html = html
...
def set_editor_html(html: str) -> None:
    ...
    _editor_window.evaluate_js(f"setEditorContent({data});")
...
def get_editor_html() -> str | None:
    ...
    if not _wait_ready():
        return None
    payload = _get_content_payload()
    return payload.get("html", "")


def get_editor_delta() -> Dict[str, Any]:
    if _fallback_text is not None:
        return {}
    if not _wait_ready():
        return {}
...
def get_selection_html() -> str:
    ...
    if not _wait_ready():
        return ""
    payload = _get_selection_payload()
    return payload.get("html", "")


def get_selection_delta() -> Dict[str, Any]:
    if _fallback_text is not None:
        return {}
    if not _wait_ready():
        return {}
...
def _create_editor_window(title: str, on_close: Optional[Callable[[], None]]) -> None:
    global _editor_window, _editor_bridge
    if _editor_window is not None:
        return
    webview = _get_webview()
    if webview is None:
        return
    _editor_ready.clear()
    _editor_bridge = QuillEditorBridge()
...
def _on_ready() -> None:
    global _pending_html
    _editor_ready.set()
    if _pending_html is not None:
        pending_html = _pending_html
        _pending_html = None
        set_editor_html(pending_html)
...
def _get_content_payload() -> Dict[str, Any]:
    if _editor_window is None:
        return {}
    result = _editor_window.evaluate_js("JSON.stringify(getEditorContent())")
    try:
...
def _get_selection_payload() -> Dict[str, Any]:
    if _editor_window is None:
        return {}
    result = _editor_window.evaluate_js("JSON.stringify(getSelectionPayload())")
    try:
        return json.loads(result) if result else {}
    except json.JSONDecodeError:
        return {}
